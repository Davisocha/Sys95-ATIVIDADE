...
namespace ComClassSys
{
    public class Pedido
    {
        ...
        //    Desconto = desconto;
        //}
        public Pedido(int id, Usuario usuario, Cliente cliente, string status, double desconto)
        {
            Id = id;
            Usuario = usuario;
            Cliente = cliente;
        ...
        //
        public static List<Pedido> ObterListaSTATUS(string status)
        {
            List<Pedido> pedidos = new();
            var cmd = Banco.Abrir();
            if (status == "")
        ...
        //
        public static List<Pedido> ObterListaCPF(string cpf)
        {
            List<Pedido> pedidos = new();
            var cmd = Banco.Abrir();
            if (cpf == "")
            ...
            {
                cmd.CommandText = $"select * from pedidos p inner join clientes c on p.cliente_id = c.id where c.cpf = '{cpf}'";
            }
        ...
        //
        public static List<Pedido> ObterListaID(int id )
        {
            List<Pedido> pedidos = new();
            var cmd = Banco.Abrir();
            if (id == 0)
